...
void * riemann_parallel(void * args)
{
	...
	for(size_t i = 0; i < shared_data->iterations_per_thread; i++)
	{
		

		thread_data->calculated_area += f(thread_data->thread_lower_limit+(i*shared_data->rectangle_width));
//		printf("Current iteration: %zu\n", i);
//		printf("Thread %zu calculated the following area = (%lf) = f( %lf + ( %zu * %lf  )\n",	thread_data->thread_num, 
//		thread_data->calculated_area, thread_data->thread_lower_limit, i, shared_data->rectangle_width);
...
//			printf("Iterations per thread: %zu\n", shared_data->iterations_per_thread);
			...
			for(size_t i = 0; i < thread_count; ++i)
			{
				...
				thread_data_list[i].shared_data = shared_data;
				thread_data_list[i].thread_lower_limit = shared_data->lower_limit + 
				(i*shared_data->rectangle_width*shared_data->iterations_per_thread);
				thread_data_list[i].thread_num = i;
//                        	printf("LOWER LIMIT SENT TO THREAD = %lf\n", thread_data_list[i].thread_lower_limit);
                        	pthread_create(&threads[i], NULL, riemann_parallel, (void*)&thread_data_list[i]);
//                        	printf("Thread %zu created\n", i);
...
//			printf("Thread count-1 completed %zu iterations\n",shared_data->iterations_per_thread*thread_count-1);
			...
			thread_data_list[thread_count-1].thread_lower_limit = shared_data->lower_limit + 
		        (shared_data->rectangle_width*(thread_count-1)*shared_data->iterations_per_thread);

			thread_data_list[thread_count-1].thread_num = thread_count-1;

//                      printf("LOWER LIMIT SENT TO THREAD = %lf\n", thread_data_list[thread_count-1].thread_lower_limit);

			pthread_create(&threads[thread_count-1], NULL, riemann_parallel, (void *)&thread_data_list[thread_count-1]);

//			printf("Final thread %zu created, loaded with %zu iterations\n", thread_data_list[thread_count-1].thread_num, 
//			shared_data->iterations_per_thread);
	...
	printf("Combined area = %lf  = %lf*%lf\n",shared_data->combined_area*
 	shared_data->rectangle_width,shared_data->rectangle_width,shared_data->combined_area);

...
GtkWidget *p1_pokemon_widgets[AMOUNT_OF_POKEMON];
GtkWidget *p2_pokemon_widgets[AMOUNT_OF_POKEMON];
...
static gboolean draw_selection_screen(GtkWidget *widget,GdkEventExpose *event,gpointer data)
{
    if (running_track) {
        ...
        for (int i = 0; i < NUM_LANES; ++i) {
            ...
            if (running_track->lanes_start_line[i]) {
                gtk_label_set_text(GTK_LABEL(p1_pokemon_widgets[i]), "READY!");
            } else {
                gtk_label_set_text(GTK_LABEL(p1_pokemon_widgets[i]), "-");
            }

            if (running_track->lanes_finish_line[i]) {
                char position[10];
                sprintf(position, "Place: %d", running_track->lanes_finish_line[i]);
                gtk_label_set_text(GTK_LABEL(p2_pokemon_widgets[i]), position);
            } else {
                gtk_label_set_text(GTK_LABEL(p2_pokemon_widgets[i]), "-");
            }
...
int main(int argc, char* argv[]) 
{
    ...
    app = gtk_application_new("example.gtk.pthreadmon", G_APPLICATION_FLAGS_NONE);
